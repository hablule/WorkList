...
namespace CustomPrintForm
{
    ...
    public class CustomPrintForm
    {
        ...
        */
        ...
        [DllImport("winspool.drv", EntryPoint = "GetForm", SetLastError = true,
            CharSet = CharSet.Unicode, ExactSpelling = false, CallingConvention = CallingConvention.StdCall),
        SuppressUnmanagedCodeSecurityAttribute()]
        internal static extern bool GetForm(IntPtr phPrinter, string pName,
            [MarshalAs(UnmanagedType.I4)] int level,
            ref FormInfo1 form, int cbBuf, out int pcbNeeded);
        ...
        public static void AddCustomPaperSizeToDefaultPrinter(string paperName, float widthMm, float heightMm)
        {
            ...
            AddCustomPaperSize(sPrinterName, paperName, widthMm, heightMm, false);
        }
        ...
        public static void AddCustomPaperSize(string printerName, string paperName, float
            widthMm, float heightMm, bool deletePaper)
        {
            if (PlatformID.Win32NT == Environment.OSVersion.Platform)
            {
                ...
                if (OpenPrinter(printerName, out hPrinter, ref defaults))
                {
                    try
                    {
                        ...
                        FormInfo1 formInfoExisting = new FormInfo1();
                        GetForm(hPrinter, pName, 1, ref formInfoExisting, 0, out iPaperInfoSize);

                        while (true)
                        {                            
                            if (!GetForm(hPrinter, pName, 1, ref formInfoExisting, iPaperInfoSize, out iPaperInfoSize))
                            {
                                break;
                            }
                            pName = otherSettings.stPaperName + (++count).ToString();
                        ...
                        formInfo.pName = pName;
                        otherSettings.stPaperName = pName;
