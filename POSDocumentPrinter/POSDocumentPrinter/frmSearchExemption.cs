...
{
    public partial class frmSearchExemption : Form
    {
        public frmSearchExemption()
        {
            InitializeComponent();
        }
        ...
        private void establishSearchCriterion()
        {
            ...
            {
                this.addMoreCriteria("DATEINVOICED", this.cmbDateLogic.SelectedItem.ToString(),
            ...
            {
                this.addMoreCriteria("EXEMPTEDAMT", this.cmbReceiptAmtLogic.SelectedItem.ToString(),
            ...
            {
                this.addMoreCriteria("SALES_ID", this.cmbInvoiceLogic.SelectedItem.ToString(),
                    this.ddgInvoice.SelectedRowKey.ToString(), "");
            }
            
            if (this.ckIsVAT.CheckState != CheckState.Indeterminate)
            {
                if (this.ckIsVAT.CheckState == CheckState.Checked)
                    this.addMoreCriteria("EXEMPTIONTYPE", "Equals to", "VAT", "");
                else if (this.ckIsVAT.CheckState == CheckState.Unchecked)
                    this.addMoreCriteria("EXEMPTIONTYPE", "Not equals to", "VAT", "");
            }

            if (this.ckIsWithholding.CheckState != CheckState.Indeterminate)
            {
                if (this.ckIsWithholding.CheckState == CheckState.Checked)
                    this.addMoreCriteria("EXEMPTIONTYPE", "Equals to", "WITHHOLDING", "");
                else if (this.ckIsWithholding.CheckState == CheckState.Unchecked)
                    this.addMoreCriteria("EXEMPTIONTYPE", "Not equals to", "WITHHOLDING", "");
            }
        }
        ...
        private void frmSearchExemption_Load(object sender, EventArgs e)
        {
        ...
        private void cmbSearchAndIncludeToPreviousResult_Click(object sender, EventArgs e)
        {
            ...
            currentSearchCriteriaResult =
                this.getDataFromDb.getV_EXEMPTIONDTLInfo
                    (this.searchQuery, this.logicalConnector, "", "", "",
                     DateTime.Now, false, exemptionType.ignor, "", "", 
                     true, "AND");

            currentSearchCriteriaResult =
                this.getDataFromDb.clearRedundantRow(currentSearchCriteriaResult, "C_EXEMPTION_ID");
                ...
                if (previousSearchResult.Columns.Count <= 0)
                {
                    ...
                    previousSearchResult =
                        this.getDataFromDb.mergeTables(previousSearchResult,
                            currentSearchCriteriaResult, "C_EXEMPTION_ID", true);
            }
            else if (considerPreviousCriteria == DialogResult.No)
            {
                previousSearchResult =
                        this.getDataFromDb.mergeTables(previousSearchResult,
                            currentSearchCriteriaResult, "C_EXEMPTION_ID", false);                
            }
