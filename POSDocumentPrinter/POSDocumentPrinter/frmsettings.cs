...
namespace POSDocumentPrinter
{
    public partial class frmsettings : Form
    {
        ...
        {            
            ...
            {
                this.readOrWriteFromXMLfile(true, this.grbDataBaseSettings);
            }
           ...
           if (read)
            {
                    ...
                    rowCounter++)
                {
                    ...
                    if (this.grbDataBaseSettings.Controls[controlName] is TextBox)
                    {
                        this.grbDataBaseSettings.Controls[controlName].Text =
                        dtSettings.Rows[rowCounter][1].ToString(); 
                    }
                    else if (this.grbDataBaseSettings.Controls[controlName] is CheckBox)
                    {
                        ((CheckBox)this.grbDataBaseSettings.Controls[controlName]).Checked =
                            Convert.ToBoolean(dtSettings.Rows[rowCounter][1].ToString());
                    }
                    else if (this.grbDataBaseSettings.Controls[controlName] is ComboBox)
                    {
                        if (((ComboBox)this.grbDataBaseSettings.Controls[controlName]).Items.Contains(dtSettings.Rows[rowCounter][1].ToString()))
                        {
                            ((ComboBox)this.grbDataBaseSettings.Controls[controlName]).SelectedItem =
                                dtSettings.Rows[rowCounter][1].ToString();
                        }
                    }
                }
            ...
            {   
                for (int controlCounter = 0;
                    controlCounter < this.grbDataBaseSettings.Controls.Count;
                    controlCounter++)
                {
                    string controlName =
        ...
        private void cmdSettingTest_Click(object sender, EventArgs e)
        {
            ...
            if (!checkAllNeccessaryFields(this.grbDataBaseSettings))
            {
        ...
        private void cmdSettingSave_Click(object sender, EventArgs e)
        {
            ...
            {
                readOrWriteFromXMLfile(false, this.grbDataBaseSettings);
        ...
        private void AllowDeliveryOrderPrinting_CheckedChanged(object sender, EventArgs e)
        {
            if (!this.AllowDeliveryOrderPrinting.Checked)
            {
                this.AutoGenerateDeliveryOrder.Checked = false;
            }
        }
