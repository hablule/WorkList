...
namespace BuySimple
{
    class dataBuilder
    {
        ...
        public string[] getCompiereTablePrimaryKey(string tableName)
        {
            ...
            { primarykeys = new string[] { "PM_C_PPAYMENT_INVOCELINE_ID" }; }
            else if (tableName == "PM_PAYMENT_INVOCESHARE")
            { primarykeys = new string[] { "PM_PAYMENT_INVOCESHARE_ID" }; }
            ...
            else if (tableName == "C_INVOICELINE_PRICES")
            { primarykeys = new string[] { "C_INVOICELINE_ID", "C_CURRENCY_ID" }; }
            ...
            bool onlyActiveRecords, bool structureOnly, string logicalConnector)
        {
            ...
            if (NAME != "")
                whereClause = whereClause + " " + logicalConnector +
                    " UPPER(NAME) LIKE '%" + NAME.Replace("'", "''").ToUpperInvariant() + "%'";
            ...
            bool structureOnly, string logicalConnector)
        {
            ...
            whereClause = whereClause + " " + logicalConnector + " ISCOST = 'Y'";
        ...
        public DataTable getC_INVOICELINE_PRICESInfo(DataTable criteriaTable, string criteriaTableLogicalConnector,
            string AD_ORG_ID, string C_INVOICELINE_ID,
            bool onlyActiveRecords, bool structureOnly, string logicalConnector)
        {
            string getRequestedTableInformation = "";
            if (structureOnly)
            {
                return this.getDataTableStructure("C_INVOICELINE_PRICES");
            }
            ...
            getRequestedTableInformation = "SELECT * " +
                                           "FROM C_INVOICELINE_PRICES " + whereClause;
        ...
        //Get Purchase Payments with distibutable amount which is not fully distributed
        public DataTable getOpenPM_C_PURCHASEPAYMENTInfo(DataTable criteriaTable, string criteriaTableLogicalConnector,
                                string AD_ORG_ID, string PM_C_PURCHASEPAYMENT_ID, string DATEINVOICED, string DOCUMENTNO,
                                string C_ORDER_ID, string C_INVOICE_ID, string C_BPARTNER_ID, 
                                string C_BANKACCOUNT_ID, string C_CASHBOOK_ID, string C_TAX_ID, string C_CHARGE_ID,
                                triaStateBool USERINSERTED, triaStateBool ISAMOUNTESTIMATE, triaStateBool ActiveRecords, string logicalConnector)
        {
            
            string whereClause = "WHERE pp.AD_CLIENT_ID = '" + generalResultInformation.clientId + 
                "' AND (pp.ISAMOUNTDISTRIBUTABLE = 'Y' AND pp.COSTCLOSED = 'N') ";

            if (ActiveRecords == triaStateBool.yes)
            {
                whereClause = whereClause + " " + logicalConnector + " pp.ISACTIVE = 'Y'";
            }
            else if (ActiveRecords == triaStateBool.no)
            {
                whereClause = whereClause + " " + logicalConnector + " pp.ISACTIVE = 'N'";
            }
            ...
            {
                whereClause = whereClause + " " + logicalConnector +
                    " pp.ISAMOUNTESTIMATE = 'Y'";
            }
            else if (ISAMOUNTESTIMATE == triaStateBool.no)
                whereClause = whereClause + " " + logicalConnector +
                    " pp.ISAMOUNTESTIMATE = 'N'";

            if (AD_ORG_ID != "")//
                whereClause = whereClause + " " + logicalConnector +
                    " pp.AD_ORG_ID = '" + AD_ORG_ID + "'";

            if (C_BANKACCOUNT_ID != "")//
                whereClause = whereClause + " " + logicalConnector +
                    " pp.C_BANKACCOUNT_ID = '" + C_BANKACCOUNT_ID + "'";

            if (C_TAX_ID != "")//
                whereClause = whereClause + " " + logicalConnector +
                    " pp.C_TAX_ID = '" + C_TAX_ID + "'";

            if (C_BPARTNER_ID != "")//
                whereClause = whereClause + " " + logicalConnector +
                    " pp.C_BPARTNER_ID = '" + C_BPARTNER_ID + "'";


            if (PM_C_PURCHASEPAYMENT_ID != "")//
                whereClause = whereClause + " " + logicalConnector +
                    " pp.PM_C_PURCHASEPAYMENT_ID = '" + PM_C_PURCHASEPAYMENT_ID + "'";


            if (C_CASHBOOK_ID != "")//
                whereClause = whereClause + " " + logicalConnector +
                    " pp.C_CASHBOOK_ID = '" + C_CASHBOOK_ID + "'";

            if (C_CHARGE_ID != "")//
                whereClause = whereClause + " " + logicalConnector +
                    " pp.C_CHARGE_ID = '" + C_CHARGE_ID + "'";

            if (C_ORDER_ID != "")//
                whereClause = whereClause + " " + logicalConnector +
                    " pp.C_ORDER_ID = '" + C_ORDER_ID + "'";

            if (DOCUMENTNO != "")//
                whereClause = whereClause + " " + logicalConnector +
                    " pp.DOCUMENTNO = '" + DOCUMENTNO.Replace("'", "''") + "'";

            if (DATEINVOICED != "")//
                whereClause = whereClause + " " + logicalConnector +
                    " pp.DATEINVOICED = '" + DATEINVOICED + "'";

           
            string getRequestedTableInformation =
                "SELECT pp.PM_C_PURCHASEPAYMENT_ID, pp.AD_CLIENT_ID, pp.AD_ORG_ID, pp.CREATED, " +
                        "pp.CREATEDBY, pp.UPDATED, pp.UPDATEDBY, pp.ISACTIVE, pp.DOCUMENTNO, " +
                        "pp.C_BPARTNER_ID, b.NAME, pp.DATEINVOICED, pp.INVOICEAMOUNT, " +
                        "pp.ISAMOUNTESTIMATE, pp.C_BANKACCOUNT_ID, pp.C_CASHBOOK_ID, " +
                        "COALESCE(bnk.ACCOUNTNO, cb.NAME) AS PAYEDFROM, pp.DESCRIPTION, " +
                        "pp.C_TAX_ID, pp.C_CHARGE_ID, COALESCE(tx.NAME, chrg.NAME) AS PAYEDFOR, " +
                        "pp.C_ORDER_ID, o.DOCUMENTNO AS ORDER_DOC, pp.C_INVOICE_ID, pp.USERINSERTED, " +
                        "pp.ISAMOUNTDISTRIBUTABLE, pp.COSTCLOSED, pp.PROCESSING, pp.PROCESSED, " +
                        "pp.PAYMENTTYPE, COALESCE(SUM(pl.AMOUNTSHARED), 0) AS AMOUNTSHARED, " +
                        "ROUND((pp.INVOICEAMOUNT - COALESCE(SUM(pl.AMOUNTSHARED), 0)), 2) AS REMAINING, " +
                        "ROUND(((pp.INVOICEAMOUNT - COALESCE(SUM(pl.AMOUNTSHARED), 0))/pp.INVOICEAMOUNT), 2) AS PRCNTG " +
                "FROM PM_C_PURCHASEPAYMENT pp " +
                        "LEFT JOIN PM_C_PPAYMENT_INVOCELINE pl ON pp.PM_C_PURCHASEPAYMENT_ID = pl.PM_C_PURCHASEPAYMENT_ID " +
                        "LEFT JOIN C_BPARTNER b ON b.C_BPARTNER_ID = pp.C_BPARTNER_ID " +
                        "LEFT JOIN C_BANKACCOUNT bnk ON bnk.C_BANKACCOUNT_ID = pp.C_BANKACCOUNT_ID " +
                        "LEFT JOIN C_CASHBOOK cb ON cb.C_CASHBOOK_ID = pp.C_CASHBOOK_ID " +
                        "LEFT JOIN C_CHARGE chrg ON chrg.C_CHARGE_ID = pp.C_CHARGE_ID " +
                        "LEFT JOIN C_TAX tx ON tx.C_TAX_ID = pp.C_TAX_ID " +
                        "LEFT JOIN C_ORDER o ON o.C_ORDER_ID = pp.C_ORDER_ID " +
                whereClause + " " +
                "GROUP BY pp.PM_C_PURCHASEPAYMENT_ID, pp.AD_CLIENT_ID, pp.AD_ORG_ID, pp.CREATED, " +
                        "pp.CREATEDBY, pp.UPDATED, pp.UPDATEDBY, pp.ISACTIVE, pp.DOCUMENTNO, " +
                        "pp.C_BPARTNER_ID, b.NAME, pp.DATEINVOICED, pp.INVOICEAMOUNT, " +
                        "pp.ISAMOUNTESTIMATE, pp.C_BANKACCOUNT_ID, pp.C_CASHBOOK_ID, " +
                        "COALESCE(bnk.ACCOUNTNO, cb.NAME), pp.DESCRIPTION, " +
                        "pp.C_TAX_ID, pp.C_CHARGE_ID, COALESCE(tx.NAME, chrg.NAME), " +
                        "pp.C_ORDER_ID, o.DOCUMENTNO, pp.C_INVOICE_ID, pp.USERINSERTED, " +
                        "pp.ISAMOUNTDISTRIBUTABLE, pp.COSTCLOSED, pp.PROCESSING, pp.PROCESSED, " +
                        "pp.PAYMENTTYPE " +
                 "HAVING pp.INVOICEAMOUNT != SUM(pl.AMOUNTSHARED) ";
        ...
        public DataTable getPM_C_PPAYMENT_INVOCELINEInfo(DataTable criteriaTable, string criteriaTableLogicalConnector,
            string AD_ORG_ID, string PM_C_PPAYMENT_INVOCELINE_ID, string PM_C_PURCHASEPAYMENT_ID, 
            string C_INVOICELINE_ID,string C_INVOICE_ID,
            ...
            bool structureOnly, string logicalConnector)
        {
            ...
            if (PM_C_PPAYMENT_INVOCELINE_ID != "")
                whereClause = whereClause + " " + logicalConnector +
                    " PM_C_PPAYMENT_INVOCELINE_ID = '" + PM_C_PPAYMENT_INVOCELINE_ID + "'";
        ...
        public DataTable getPM_PAYMENT_INVOCESHAREInfo(DataTable criteriaTable, string criteriaTableLogicalConnector,
            string AD_ORG_ID, string PM_PAYMENT_INVOCESHARE_ID, string PM_C_PURCHASEPAYMENT_ID, string C_INVOICE_ID, 
            ...
            string logicalConnector)
        {
            ...
            if (PM_PAYMENT_INVOCESHARE_ID != "")//
                whereClause = whereClause + " " + logicalConnector +
                    " PM_PAYMENT_INVOCESHARE_ID = '" + PM_PAYMENT_INVOCESHARE_ID + "'";
